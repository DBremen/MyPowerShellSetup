...
function Invoke-FzfPsReadlineHandlerProvider {
    ...
    try {
        if (-not [System.String]::IsNullOrWhiteSpace($env:FZF_CTRL_T_COMMAND)) {
            Invoke-Expression ($env:FZF_CTRL_T_COMMAND) | Invoke-Fzf -Multi -previewwindow 'right:60%' -preview 'bat --color=always --style=full --line-range :300 {}' | ForEach-Object { $result += $_ }
        }
        else {
            if ([string]::IsNullOrWhiteSpace($currentPath)) {
                ...
                switch ($providerName) {
                    ...
                    'FileSystem' { Invoke-Expression ($script:ShellCmd -f ($fileSystemCmd -f $resolvedPath.ProviderPath)) | Invoke-Fzf -Multi -previewwindow 'right:60%' -preview 'bat --color=always --style=full --line-range :300 {}' | ForEach-Object { $result += $_ } }
