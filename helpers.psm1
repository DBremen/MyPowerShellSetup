...
# remember to remove the menu alias from showui del alias:menu -Force
function cdd { 
...
function cds {
    $location = ([System.Enum]::GetNames([System.Environment+SpecialFolder]) | fzf | ForEach-Object { [System.Environment]::GetFolderPath($_) })
    if ($location) {
        cd $location
    }
}
...
function Format-XML {
    ...
    begin {
        #to cope with multiple items being piped we will assemble blocks of XML or Lumps of text in the process block and output them in the end block
        [xml[]]$xmlblocks = @() 
        [String[]]$text = @()     
    }
    process {
