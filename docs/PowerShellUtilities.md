...
| Terminal-Icons | module | [GitHub](https://github.com/devblackops/Terminal-Icons) | Provides custom terminal icons. Modified [Terminal-Icons.format.ps1xml](https://github.com/DBremen/MyPowerShellSetup/blob/master/Terminal-Icons.format.ps1xml). (On Windows replace 'LF' by 'CRLF' ```((Get-Content $path) -join "`r`n") + "`r`n" | Set-Content -NoNewline $path -encoding utf8)```) file to Format Length output into B,KB,MB,GB  | Install-Module -Name Terminal-Icons; get appropriate font (see [quick guide by Mark Wragg](https://gist.github.com/markwragg/6301bfcd56ce86c3de2bd7e2f09a8839) |
